function valid_datas( f ){
	...
	if( f.name.value == '' ){
		...
		jQuery('#form_status').html('<span class="wrong">O campo EMAIL não pode estar vazio e deve ser preenchido corretamente!</span>');
		notice( f.email );
	//}else if( f.phone.value == '' ){
		//jQuery('#form_status').html('<span class="wrong">Your phone must not be empty and correct format!</span>');
		//notice( f.phone );
	}else if( f.subject.value == '' ){
		jQuery('#form_status').html('<span class="wrong">O campo ASSUNTO não pode estar vazio!</span>');
		notice( f.subject );
	}else if( f.message.value == '' ){
		jQuery('#form_status').html('<span class="wrong">O campo MENSAGEM não pode estar vazio!</span>');
		notice( f.message );
	}else{
		 jQuery.ajax({
			url: 'mail.php',
